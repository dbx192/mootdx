...
def fq_factor(symbol: str, method: str, ) -> pd.DataFrame:
    ...
    method_sina = 'hfq' if method in ['hfq', 'bfq'] else 'qfq'
    cache_file = get_config_path(f'caches/factor/{symbol}_{method_sina}.plk')
    ...
    def _factor(symbol: str, method: str, ) -> pd.DataFrame:
        try:
            ...
            rsp = httpx.get(url.format(symbol, method_sina))
        ...
        if res.shape[0] == 0:
            raise ValueError(f'sina {method_sina} factor not available')
